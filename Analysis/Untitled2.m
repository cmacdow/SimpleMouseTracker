...
for cur_fn = 1:length(config_fns)
    ...
    group = whichGroup(temp.MouseNum);
     if group == true
       ...
       group1_empty = horzcat(group1_empty,length(temp.CompartmentTimes{1,2}) / (30));
     else
       group0_SA = horzcat(group0_SA, length(temp.CompartmentTimes{1,1}) / (30)); % 30 fps, 60 sec per min
       group0_empty = horzcat(group0_empty, length(temp.CompartmentTimes{1,2}) / (30));
     end  
    figure
...
data = [mean(group0_SA), mean(group0_empty); mean(group1_SA), mean(group1_empty)];
bar(X, data)
ylabel('Seconds')
% sem, and indiv data points
% times go across, travel the edges vs. travel middle (proxy for anxiety)
