...
gp.sbatch_time = 59;
gp.sbatch_exclude = 'redshirt-n[12-49]';
gp.sbatch_memory = 16;
gp.sbatch_matlabversion = 'R2020b';
gp.sbatch_path = "/jukebox/buschman/Projects/Cortical Dynamics/Mouse Models of Autism/Analysis Code/SimpleMouseTracker/";
gp.sbatch_name = 'msTrack'; 
gp.local_bucket = 'Z:\Projects\Cortical Dynamics\Mouse Models of Autism\Analysis Code\SimpleMouseTracker\Spock\';
gp.delete_singlefiles = 0; %flag to delete individual recording sections after done concatenating
gp = ParseOptionalInputs(gp,varargin);

%keep this local for right now;
script_name = sprintf('run_%s.sh',uniqID);
file_path = gp.local_bucket;
if ~exist(file_path)
    mkdir(file_path);
end
...
fprintf(fid,"\n%s",'module load gstreamer/1.14.0'); 
